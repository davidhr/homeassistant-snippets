...
# In order to avoid the 'Too many levels of symbolic links (40)' message on the Raspberry:
...
red="0000"
#red_state=`awk 'FNR==1' /tmp/blebulb.state`
red_state=0
green="0000"
#green_state=`awk 'FNR==2' /tmp/blebulb.state`
green_state=0
blue="0000"
#blue_state=`awk 'FNR==3' /tmp/blebulb.state`
blue_state=0
white="01FF"
white_state=255
...
function setcolor {
    case "$1" in
	aqua)	setrgb 0 255 255 0;;
	black)	setrgb 0 0 0 0;;  # WTF?!
	blue)	setrgb 0 0 255 0;;
	fuchsia) setrgb 255 0 255 0;;
	gray)	setrgb 128 128 128 0;;
	green)	setrgb 0 128 0 0;;
	lime)	setrgb 0 255 0 0;;
	maroon)	setrgb 128 0 0 0;;
	navy)	setrgb 0 0 128 0;;
	olive)	setrgb 128 128 0 0;;
	purple)	setrgb 128 0 128 0;;
	red)	setrgb 255 0 0 0;;
	silver)	setrgb 192 192 192 0;;
	teal)	setrgb 0 128 128 0;;
	white)	setrgb 255 255 255 0;;
	yellow)	setrgb 255 255 0 0;;
...
function display_help {
    help_text="Usage: blebulb MAC_ADDRESS [OPTION]... [VALUE]...\n
...
case "$2" in
    -s|--set)	setrgb $3 $4 $5 $6 && storevalues;;
    -f|--fade)	fade $3 $4 $5 $6 && storevalues;;
    -c|--color)	setcolor $3 && storevalues;;
    
	--current)	echo $red_state $green_state $blue_state $white_state;;
    *)		echo -e "blebulb: invalid option -- '$2'\nTry 'blebulb --help' for more information.";;
esac
